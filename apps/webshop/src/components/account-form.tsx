...
import {
  ...
  User as UserIcon, Mail, Lock, Settings, BookOpen, ShoppingBag, Package
} from "lucide-react";
...
export default function AccountForm({ user, orders }: AccountFormProps) {
  ...
  // Password state — keeping your current/new/confirm structure
                ...
                {orders?.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                    ...
                    <p className="text-xs font-body text-outline tracking-wide">
                      When you purchase something, it'll show up here.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {orders.map((order) => (
                      <div
                        key={order.id}
                        className="border border-outline-variant/20 bg-surface-container-low p-5 space-y-3"
                      >
                        {/* Order header */}
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div>
                            <p className="text-xs font-label uppercase tracking-widest text-outline mb-1">Order</p>
                            <p className="font-label text-sm text-on-surface truncate max-w-[200px]" title={order.id}>
                              #{order.id.slice(0, 8)}…
                            </p>
                          </div>
                          ...
                          {order.items.map((item) => (
                            <div key={item.id} className="flex justify-between items-baseline gap-4">
                              ...
                              <p className="text-xs font-label text-outline shrink-0">
                                {(item.unitPrice / 100).toLocaleString("sv-SE", {
                                  style: "currency",
                        ...
                        <div className="border-t border-outline-variant/10 pt-3 flex justify-between items-center flex-wrap gap-2">
                          ...
                          <p className="text-sm font-label text-on-surface">
                            ...
                            <span className="text-primary font-semibold">
                              {(order.totalAmount / 100).toLocaleString("sv-SE", {
                                style: "currency",
                                currency: order.currency.toUpperCase(),
                              })}
                            </span>
