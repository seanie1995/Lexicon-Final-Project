...
const BlogPostContent = ({ post }: BlogPostContentProps) => {
	return (
		<main className="min-h-screen bg-background pt-24">
			{/* Hero Section */}
			<section className="relative w-full h-[45vh] md:h-[600px] overflow-hidden">
				<Image
					src={post.image}
					alt={post.title}
					fill
			...
			{/* Article Header */}
			<header className="max-w-4xl mx-auto px-6 -mt-32 relative z-10 text-center">
				<div className="bg-surface p-8 md:p-12 shadow-sm border border-outline-variant/10">
					<nav className="mb-8 flex justify-center items-center gap-4 text-xs font-label tracking-[0.2em] text-secondary uppercase">
						<span className="cursor-pointer hover:text-primary transition-colors">The Archivist's Journal</span>
			...
			<article className="max-w-3xl mx-auto px-6 py-20 pb-40">
				<div
					...
					dangerouslySetInnerHTML={{ __html: post.body.split('\n').filter(p => p.trim()).map(p => `<p>${p.trim()}</p>`).join('') }}
				/>
			</article>
		</main>
	);
