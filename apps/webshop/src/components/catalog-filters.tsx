	...
	if (type === "radio") {
		return (
			<label
				...
				onClick={(e) => {
					// We drive state via URL query params; prevent the input from
					// attempting to manage its own checked state.
					e.preventDefault();
					onToggle();
				}}
			>
...
function buildUrl({
	...
	// Reset pagination if/when you add it later.
	next.delete("page");
									...
									return (
										<li key={option}>
											<FilterOption
												...
												onToggle={() => {
													const nextChecked =
														section.type === "checkbox" ? !currentChecked : true;
													const href = buildUrl({
														pathname,
